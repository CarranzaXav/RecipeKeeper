...
const ForgotPassword = () => {
    ...
    const onSubmit = async (e) => {
        ...
        setErrMsg('');
        try {
            const payload = await requestPasscode({ phone }).unwrap();
            // payload: {otpUserId, maskedPhone, username}
            navigate('/verify-passcode', {state: payload, replace: true});
        } catch (err) {
        ...
        >
            ...
            <main>
                ...
                <form onSubmit={onSubmit} className="forgotPwdForm">
                    <div className="forgotPwdPhoneBlock">
                        <label htmlFor="phone" className="forgotPwdLabel">Phone Number:</label>
                        <input 
                            type="tel" 
                            id="phone" 
            ...
            <footer className="forgotPwdFooter">
                <div className="forgotPwdFooterBtnContainer">
                    <Link className='loginFormButton' to='/login'>Back to Login</Link>
                </div>
            </footer>
        </section>
    </div>
