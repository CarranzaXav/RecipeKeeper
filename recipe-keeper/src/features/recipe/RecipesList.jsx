...
const RecipesList = () => {
  ...
  if (isLoading)
    return (
      <div className="flex mt-24 h-96 justify-center">
        <Loader />
      </div>
    );
  ...
  if (isSuccess) {
    ...
    const recipeContent = ids?.length
      ? ids
          ...
          .map((recipeCardId) => (
            <RecipeCard key={recipeCardId} recipeCardId={recipeCardId} />
          ))
      : null;
